...
export const qrCodes = pgTable('qr_codes', {
  ...
  paymongoLinkId: text('paymongo_link_id').notNull(), // Paymongo payment link ID
  paymongoLinkUrl: text('paymongo_link_url').notNull(), // Paymongo payment link URL
  status: varchar('status', { length: 20 }).notNull().default('active'), // active, expired, used, invalidated
